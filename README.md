...
- ```git log --graph --oneline``` - с флагом ```--graph``` Git нарисует ветки с помощью «палочек» и «звёздочек»
- ```git log --graph --oneline <название_ветки_1> <название_ветки_2>``` - команде git log можно указать несколько веток, и тогда она выведет их все
...
- ```git push --force``` - принудительно отправить локальную ветку в удалённую, даже если они не в состоянии Fast-forward, ВОЗМОЖНА ПОТЕРЯ ИЗМЕНЕНИЙ ИЗ УДАЛЁННОГО РЕПОЗИТОРИЯ
...
- ```git merge --no-ff <название_ветки>``` — выполнить слияние, ```--no-ff``` - отключить Fast-forward слияние
- ```git merge --no-edit <название_ветки>``` — выполнить слияние, ```--no-edit``` - избавляет от необходимости вводить сообщение для merge-коммита
- ```git merge --abort``` — отменить начатое слияние, не завершенное из-за конфликтов
- ```git mergetool``` — запустить утилиту командной строки для разрешения конфликтов (по умолчанию ```vimdiff```)
- ```git config [--global] merge.ff false``` - отключить Fast-forward слияние в настройках Git
...
- ```git config --global alias.<алиас> "<команда>"``` — создать алиас для команды, например: ```git config --global alias.gl "log --oneline --abbrev-commit --graph --date=short --pretty=format:'%h - %an, %cd : %s'"
```
---
